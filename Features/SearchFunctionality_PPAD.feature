Feature: Search Functionality in PPAD
 ...
 # Background: 
  ...
  #YPS-6727
  Scenario: Verify for Non Logged IN User If there are no matching results and coveo fails to make any suggestion then below message will be displayed which is CMS editable
    Given user is on Home Page
    And User clicks on search icon in header
    When User clicks anywhere in search bar in site
    Then User should be able to type search "xyz" in site
